...
class Lattice(object):
    ...
    def action(self):
        ...
        """
        return 1/(2)*np.sum(self.I(self.lattice)**2)
    ...
    def I(self, lattice):
        ...
        for i in range(self.dim):
            ...
            result += forward + backward - 2*lattice + np.sqrt(np.abs(self.lambda_))*(forward-lattice)**2
        return result 
    ...
    def actions(self, lattice):
        ...
        """
        return 1/(2)*np.sum(self.I(lattice)**2)
    ...
    def dot_p(self, lattice):
        ...
        result = 0
        for i in range(self.dim):
            Foward = np.roll(I,-1,axis = i)*1
            Backward = np.roll(I,1,axis = i)*(1 + 2*np.sqrt(np.abs(self.lambda_))*lattice - 2*np.sqrt(np.abs(self.lambda_))*np.roll(lattice, 1, i))
            Centre = I*(2*np.sqrt(np.abs(self.lambda_))*lattice - 2 - 2*np.sqrt(np.abs(self.lambda_))*np.roll(lattice, -1, i))
            result += Foward + Backward + Centre
        return -1*result # return the evolution of the momentum, don't forget the minus sign
